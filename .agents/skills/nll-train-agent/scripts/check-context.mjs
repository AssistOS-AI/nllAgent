...
requirePopulated(context, "ontology", diagnostics,
  "the selected theory requires at least one ontology signature");
requirePopulated(context, "theorySources", diagnostics,
  "training requires at least one indexed authority source");
if (context.methodCatalog === null || context.methodCatalog === undefined) {
  diagnostics.push(diagnostic("CTX_EMPTY", "methodCatalog: training requires the available method catalog"));
}
requirePopulated(context, "providers", diagnostics,
  "training requires the pinned or reusable provider catalog");
requirePopulated(context, "sdkImports", diagnostics,
  "training requires explicit SDK imports");
requirePopulated(context, "commands", diagnostics,
  "training requires host validation commands");
...
function finish(value, found) {
  ...
  process.stdout.write(`theory sources: ${collectionSize(value.theorySources)}\n`);
  process.stdout.write(`available circuits: ${collectionSize(value.circuits)}\n`);
  process.stdout.write(`SDK imports: ${collectionSize(value.sdkImports)}\n`);
}
