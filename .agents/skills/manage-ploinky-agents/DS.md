...
The skill preserves a fixed set of invariants: the canonical agent id `agent:<repo>/<agent>` is durable while route keys and aliases are only labels; the router is the single public control point for agent application surfaces, with a declared media or data plane (for example a WebRTC SFU) as the narrow, control-plane-gated exception; the session, agent assertion, router request, and user delegation token families flow in fixed directions and bind internal requests to a recomputed request hash; MCP policy is explicit and fail-closed; HTTP route access is path-based, uses only `public`, `guest`, or `authenticated`, keeps `public` readonly, and remains separate from MCP policy; and chat completions stay non-privileged. Secrets stay separated, so an agent never receives the master key or another agent's secret.
