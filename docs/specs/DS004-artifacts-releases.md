...
- benchmark policy plus the identities and results of suites used for qualification;
- an immutable snapshot of every natural benchmark case actually executed by the gate;
...
- known limitations and qualification report;
- reproduction recipe and rollback metadata.

The release directory becomes immutable after qualification. Qualification must verify every declared file digest, validate all critical observation consumers against structural, extraction, or adapter producers, and prove that both candidate files and agent benchmark files remained stable while tests ran. Publication copies the tested candidate and benchmark cases into a temporary release directory, rechecks both copied snapshots, adds trusted gate artifacts, and atomically renames it into the immutable version path. A failed copy or build removes only that temporary directory and cannot leave a partial version. The derived contracts, alignment matrix, and benchmark results are copied into the release even when the author did not provide separate files. Loading checks both directions: every declared file must exist with the qualified digest, and every regular file other than `release.json` and `qualification.json` must appear in the manifest. Activation writes an atomic pointer containing the selected release identifier and manifest digest.

## Versioning and lineage

A patch release fixes an implementation defect without changing valid theory outcomes. A minor release adds compatible rules, types, document profiles, or outputs. A major release changes the meaning of a type, coverage policy, rule, authority ordering, expected outcome, or compatibility guarantee.
...
### Question #2: Are qualified releases physically write-protected?

Response: The library records local file digests at qualification, refuses in-place release writes through its API, and checks the snapshot before loading or activating it. The mechanism is deliberately simple and detects accidental or out-of-band changes in the project workspace.
...
### Question #4: Why snapshot candidate files both before and after qualification?

Response: Benchmark success is meaningful only for the exact bytes that become the release. Comparing the candidate tree before and after the gate prevents concurrent or agentic edits from replacing a tested circuit between execution and immutable publication.

### Question #5: Which planning artifacts belong to a release rather than a run?

Response: A release may contain `planningCircuits`. Qualification freezes and hashes them with schemas, validation circuits, authority mappings, and benchmarks. A run owns the idea-specific CNL plan and any optional realizations, captures, and validation attempts. Changing plan structure, content-selection logic, applied rule provenance, rule-to-plan witness semantics, or planning-circuit behavior is a semantic release change.

### Question #6: Does the canonical kind rename rewrite qualified releases?

Response: No. New candidates use `NaturalLanguageLinterRelease`, and the rename is introduced by a major release because it changes persisted vocabulary. Older qualified snapshots retain their original bytes and digests. Loading one for replay does not make its historical spelling valid for a new candidate.
