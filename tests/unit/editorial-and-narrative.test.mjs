...
test('frequency threshold is scoped to a paragraph and verifies every exact occurrence', () => {
  const program = compileMarkdown('Parcă unu, parcă doi, parcă trei.\n\nParcă separat.\n', { language: 'ro' });
  const observations = program.observations.filter((item) => item.type === 'document.paragraph@1');
  const candidates = frequencyThreshold({
    observations,
    rules: [{ id: 'ED-002', term: 'parcă', maximum: 2, wholeWord: true, caseSensitive: false }]
  }, { program });
...
test('object continuity preserves a gap but a recovery transition closes it', () => {
  const program = compileMarkdown('Mara lăsă telefonul.\n\nMai târziu îl folosi.\n', { language: 'ro' });
  const anchors = program.blocks.filter((block) => block.kind === 'paragraph').map((block) => block.anchor.id);
  const events = [
    { id: 'event:leave', type: 'narrative.object-event@1', status: 'proposed', scope: 'view:whole', anchors: [anchors[0]], payload: { objectId: 'phone:mara', action: 'leave', order: 1 } },
    { id: 'event:use', type: 'narrative.object-event@1', status: 'proposed', scope: 'view:whole', anchors: [anchors[1]], payload: { objectId: 'phone:mara', action: 'use', order: 3 } }
  ];
  ...
  const recovery = { id: 'event:retrieve', type: 'narrative.object-event@1', status: 'proposed', scope: 'view:whole', anchors: [anchors[1]], payload: { objectId: 'phone:mara', action: 'retrieve', order: 2 } };
...
test('exact match verifier independently enforces whole-word scope', () => {
  const program = compileMarkdown('Faptul rămâne.', { language: 'ro' });
  ...
  const [result] = verifier.execute({
    candidates: [{
      ...
      witness: {
        ...
        caseSensitive: false, wholeWord: true, locale: 'ro',
